...
import { Save, ArrowLeft, FileText, Brain } from 'lucide-react';
...
const AdminPage: React.FC<AdminPageProps> = ({ settings, onSave, onBack }) => {
  ...
  const [activeTab, setActiveTab] = useState<'geral' | 'cerebro'>('geral');
      ...
      <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <button onClick={onBack} className="p-2 bg-white border border-gray-100 rounded-xl text-gray-400 hover:text-gray-600 shadow-sm transition-colors">
              <ArrowLeft className="w-5 h-5" />
            </button>
            <div className="text-left">
              <h1 className="text-2xl font-bold text-gray-950 font-serif">Cérebro da IA</h1>
              <p className="text-xs text-gray-400 mt-1">Gerencie as diretrizes, tom de voz, regras e limites do robô jurídico.</p>
            </div>
          </div>
          <button 
            onClick={handleSave} 
            disabled={isSaving}
            className="w-full sm:w-auto bg-[#C5A059] hover:bg-[#C5A059]/90 disabled:opacity-50 text-white px-6 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-champagne/10"
          >
        ...
        {/* Tab Selector */}
        <div className="flex border-b border-gray-200 gap-2 pb-px overflow-x-auto">
          <button 
            onClick={() => setActiveTab('geral')}
            className={`flex items-center gap-2 px-4 py-3 text-sm font-semibold border-b-2 transition-colors whitespace-nowrap ${activeTab === 'geral' ? 'border-[#C5A059] text-champagne' : 'border-transparent text-gray-400 hover:text-gray-600 hover:border-gray-300'}`}
          >
            <FileText className="w-4 h-4" /> Dados do Escritório
          </button>
          <button 
            onClick={() => setActiveTab('cerebro')}
            className={`flex items-center gap-2 px-4 py-3 text-sm font-semibold border-b-2 transition-colors whitespace-nowrap ${activeTab === 'cerebro' ? 'border-[#C5A059] text-champagne' : 'border-transparent text-gray-400 hover:text-gray-600 hover:border-gray-300'}`}
          >
            <Brain className="w-4 h-4" /> Comportamento e Prompts
          </button>
        </div>
        ...
        <div className="bg-white border border-gray-100 p-8 rounded-3xl shadow-sm space-y-6">
          {activeTab === 'geral' && (
            <div className="space-y-6">
              <div className="text-left">
                <h3 className="text-base font-bold text-gray-900 mb-1">Identidade Jurídica</h3>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2 text-left">
                  ...
                  <input 
                    ...
                    className="w-full bg-gray-50 border border-gray-100 p-3 rounded-xl outline-none text-sm transition-colors text-gray-900 focus:bg-white" 
                    placeholder="Nome" 
                  />
                </div>
                <div className="space-y-2 text-left">
              ...
              <div className="space-y-2 text-left">
                ...
                <textarea 
                  ...
                  className="w-full bg-gray-50 border border-gray-100 p-3 rounded-xl outline-none text-sm transition-colors resize-none text-gray-900 focus:bg-white" 
          ...
          {activeTab === 'cerebro' && (
            <div className="space-y-6">
              <div className="text-left">
                <h3 className="text-base font-bold text-gray-900 mb-1">Cérebro e Prompts</h3>
